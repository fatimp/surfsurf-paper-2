...
plt.plot(x, ss(x, R))
plt.plot(np.linspace(0, 0.5, data3.size), data3 * 450**2)
plt.plot(np.linspace(0, 0.5, data5.size), data5 * 450**2)
plt.ylim([0, 10])
plt.xlim([0.03, 0.5])
plt.xlabel('$r$')
plt.ylabel('$F_{ss}(r)$')
plt.legend(['Therory', 'Filter $H$', "Filter $H'$"])
plt.savefig('../images/ball-3d-ss.png', bbox_inches = 'tight')
