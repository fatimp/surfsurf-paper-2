...
images of porous media. A more detailed description can be found in the recent
paper of Samarin et~al. \textcolor{red}{cite paper} This algorithm can calculate
...
accuracy in general was still a question. In this section we present additional
test case and develop an algorithm for calculation of $F_{ss}$ and $F_{sss}$
functions for two- and three-dimensional sets with smooth boundary.
...
acceptance criterion $C_{0.5} > 0.97$ developed in \textcolor{red}{cite our
  paper again} works also for $H'$, but sometimes when the criterion is not
satisfied, it's possible to calculate a poor estimate of $F_{ss}$ using $H$ but
not $H'$. For example, consider images of a disk used for calculation of
$F_{ss}$ on \cref{fig:fss-disk}. In \cref{table:crit} we provide values of
$C_{0.5}$ criterion and errors \cref{eq:error} for filters $H$ and $H'$.
...
\begin{table}[h!]
  ...
  \begin{tabular}{|c | c | c | c|}
    \hline
    Dimensions & $C_{0.5}$ & Error ($H$) & Error ($H'$) \\
    \hline
    $100 \times 100$ & 0.822 & 0.178 & 0.408 \\
    \hline
    $400 \times 400$ & 0.921 & 0.072 & 0.098 \\
    \hline
    $3100 \times 3100$ & 0.972 & 0.049 & 0.009 \\
    \hline
  \end{tabular}
  \caption{$C_{0.5}$ criterion and relative errors in computation of $F_{ss}$
    for filters $H$ and $H'$. The case with a disk. Plots of $F_{ss}$ functions
    can be seen on \cref{fig:fss-disk}.}
  \label{table:crit}
\end{table}
