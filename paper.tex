...
\cref{sec:cool-algo}. The choice of verification cases is presented in
\cref{sec:fss-2d} followed by improvements in methodology of Samarin et al. The
...
\end{align}
Here the brackets $\langle \dots \rangle$ denote ensemble average. For isotropic
homogeneous media vector values $\bm{x}$ and $\bm{r}$ can be replaced with
scalars. In case correlation functions are computed in predefined directions,
scalar values can be also used for anisotropic structures
\cite{jiao2014chawla,EPL1}.

More detailed information on surface correlation functions can be found in
comprehensive Torquato's book \cite{Torquato_book}.
