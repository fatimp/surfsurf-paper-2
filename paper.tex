...
\begin{abstract}
  ...
  present a novel exact algorithm to compute $F_{ss}$ and $F_{sss}$ functions
  for smooth boundary sets without discretization. This allows us to test
  digital computational approach on geometries beyond those available as analytical
  solutions. Moreover, we propose a better edge detection filter that allowed to
  improve the quality of surface correlation functions (both surface-surface and
  surface-void) assessment based on digital images. The code for continuous and
...
\begin{keyword}
  3D images, correlation functions, surface-surface function,
