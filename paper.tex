...
$H$ has faster convergence with increase of image resolution. We developed an
acceptance parameter $C_{0.5}$ to determine if an image is suitable for
processing with our algorithm or not. This parameter is defined as
\begin{equation*}
...
example, consider images of a disk used for calculation of $F_{ss}$ for
\cref{fig:fss-disk}. On \cref{fig:error} we provide values of $C_{0.5}$ for
those images and relative errors in $F_{ss}$ \cref{eq:error} for both filters.
